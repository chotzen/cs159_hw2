...
class FTOCP(object):
	...
	def buildCost(self):
		...
		listX_g = [goal] * (self.N) + [np.zeros(H.shape[0] - goal.shape[0] * self.N)] 
		# code.interact(local=locals())
		vecX_g = np.hstack(tuple(listX_g))
		q = -2 * np.matmul(vecX_g, H)
	...
	def buildEqConstr(self):
		...
		for k in range(0, self.N):
			...
			if k == 0:
				...
				Gx[k * A.shape[0]:(k+1) * A.shape[0], (k-1) * A.shape[1]:k*A.shape[1]] = -A
			Gu[k * B.shape[0]:(k+1) * B.shape[0], k * B.shape[1]:(k+1) * B.shape[1]] = -B
